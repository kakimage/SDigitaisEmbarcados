...
void MCP23017_configura_pino (uint8_t pino, uint8_t tipo)
{
	...
	i2c_start();
		resposta=i2c_le_byte(DISPOSITIVO, porta);
	i2c_stop();
	...
	i2c_start();
		 i2c_escreve_byte(DISPOSITIVO, porta, resposta);
	i2c_stop();
}
...
void MCP23017_escreve_pino (uint8_t pino, uint8_t valor)
{
    ...
    i2c_start();
		retorno=i2c_le_byte(DISPOSITIVO, latch);
	i2c_stop();
    ...
    i2c_start();
		 i2c_escreve_byte(DISPOSITIVO,porta, retorno);
	i2c_stop();
...
uint8_t MCP23017_leia_pino (uint8_t pino)
{
   ...
   i2c_start();
		retorno=i2c_le_byte(DISPOSITIVO, porta);
	i2c_stop();
