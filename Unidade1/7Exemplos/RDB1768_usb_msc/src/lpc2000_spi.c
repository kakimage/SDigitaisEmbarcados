...
// CodeRed - point to NXP header file
...
#define SELECT_CARD()   FIO0CLR = (1 << SPI_SSEL_PIN)
#define UNSELECT_CARD() FIO0SET = (1 << SPI_SSEL_PIN)
...
// ** CodeRed - new SPI setup code	
	...
	// CodeRed - clock
	PCLKSEL0 &= ~(3<<16);  // PCLK_SPI
	PCLKSEL0 |=  (1<<16);  // PCLK_periph = CCLK
	...
	//// reset Pin-Functions
	// P0.15 set to SCK
	PINSEL0 &= ~SPI_SCK_FUNCMASK;
	PINSEL0 |=  SPI_SCK_FUNCBIT;	
    ...
    PINSEL1 = PINSEL1 & ~(SPI_MOSI_FUNCMASK);
    FIO0DIR |= (1<<SPI_MOSI_PIN);
    FIO0SET = (1<<SPI_MOSI_PIN);
    ...
    PINSEL1 = PINSEL1 | (SPI_MOSI_FUNCBIT);
